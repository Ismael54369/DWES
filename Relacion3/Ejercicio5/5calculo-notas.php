    ...
    <div class="container mt-5">
        <div class="card shadow">
            ...
            <div class="card-body bg-light text-dark">
                ...
                $calificaciones = [
                    // Usamos filter_input para validar que las notas son números enteros.
                    "Inicial" => filter_input(INPUT_GET, 'notaIni', FILTER_VALIDATE_INT),
                    "Primera" => filter_input(INPUT_GET, 'nota1', FILTER_VALIDATE_INT),
                    "Segunda" => filter_input(INPUT_GET, 'nota2', FILTER_VALIDATE_INT),
                    "Tercera" => filter_input(INPUT_GET, 'nota3', FILTER_VALIDATE_INT)
                ];
                
                // Validamos y saneamos el resto de campos.
                $faltas = filter_input(INPUT_GET, 'faltas', FILTER_VALIDATE_INT);
                $nombre = htmlspecialchars($_GET['nombre']);
                $email = filter_input(INPUT_GET, 'email', FILTER_VALIDATE_EMAIL);
                $tipo_doc = htmlspecialchars($_GET['tipo_doc']);
                $num_doc = htmlspecialchars($_GET['num_doc']);

                $notaFinal = 0;
                // Calculamos la nota final
                foreach ($rubrica as $prueba => $peso) {
                    // Nos aseguramos de que la calificación es válida antes de calcular
                    if ($calificaciones[$prueba] !== false) $notaFinal += $calificaciones[$prueba] * $peso;
                }
        ...
        <a href="./GonzalezIsmael35.php" class="btn btn-outline-light mt-3">Volver al formulario</a>
    </div>
</body>
